  ...
  run(
    ...
    return runCodex(
      ...
      workDir,
      {
        ...
        onGeneratedImage: callbacks.onGeneratedImage,
        onGeneratedFile: callbacks.onGeneratedFile,
        onComplete: (raw) => {
        ...
        onError: (err) => {
          ...
          const friendly =
            msg.includes("Authentication") || msg.includes("login")
              ? "Codex 需要先登录。请在终端运行 codex login，或在 ~/.relaydesk/config.json 的 env 中添加 OPENAI_API_KEY。"
