...
vi.mock("./client.js", () => ({
  ...
  createAndDeliverCard: vi.fn(),
  updateCardInstance: vi.fn(),
  sendRobotInteractiveCard: vi.fn(),
  updateRobotInteractiveCard: vi.fn(),
}));
...
describe("DingTalk message sender", () => {
  beforeEach(() => {
    ...
    uploadMediaMock.mockResolvedValue({ mediaId: "media-1", type: "image" });
    sendMediaMock.mockResolvedValue(undefined);
  });
  ...
  it("sends image replies through the native DingTalk media path", async () => {
    const sender = await import("./message-sender.js");
    ...
    expect(uploadMediaMock).toHaveBeenCalledWith("C:\\images\\out.png", "image");
  ...
  it("falls back to text when native image send fails", async () => {
    sendMediaMock.mockRejectedValueOnce(new Error("native send failed"));
  ...
  it("sends file replies through the native DingTalk media path", async () => {
    uploadMediaMock.mockResolvedValueOnce({ mediaId: "media-file", type: "file" });

    const sender = await import("./message-sender.js");
    await sender.sendFileReply("cid-file", "C:\\files\\report.txt", {
      chatId: "cid-file",
      conversationType: "1",
      senderStaffId: "staff-1",
      robotCode: "robot-1",
    ...
    expect(uploadMediaMock).toHaveBeenCalledWith("C:\\files\\report.txt", "file");
    expect(sendMediaMock).toHaveBeenCalledWith(
      {
        chatId: "cid-file",
        conversationType: "1",
  ...
  it("falls back to text when native file target is unavailable", async () => {
    const sender = await import("./message-sender.js");
    await sender.sendFileReply("cid-file", "C:\\files\\report.txt");
    ...
    expect(sendTextMock).toHaveBeenCalledWith(
      "cid-file",
      expect.stringContaining("Generated file saved at: C:\\files\\report.txt"),
    );
  });
});
