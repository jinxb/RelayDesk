...
import { MAX_STREAMING_CONTENT_LENGTH, MAX_FEISHU_MESSAGE_LENGTH } from '../../../state/src/index.js';
import {
  RELAYDESK_BRAND_SUFFIX,
  getAIToolDisplayName,
  splitLongContent as sharedSplitLongContent,
  truncateText,
...
function getHeaderTitle(status: CardStatus, toolName: string): string {
  ...
  return `${base}${RELAYDESK_BRAND_SUFFIX}`;
}
