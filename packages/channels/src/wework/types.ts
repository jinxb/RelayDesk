...
type WeWorkNativeReplyType = 'file' | 'voice' | 'video';
type WeWorkNativeMediaBody = {
  media_id: string;
};
...
export interface WeWorkResponseMessage extends WeWorkRequest {
  ...
  body: {
    msgtype: 'stream' | 'text' | 'markdown' | WeWorkNativeReplyType;
    stream?: {
      id: string;
      finish: boolean;
      content: string;
    ...
    file?: WeWorkNativeMediaBody;
    voice?: WeWorkNativeMediaBody;
    video?: WeWorkNativeMediaBody;
  };
}
...
export interface WeWorkHttpResponseBody {
  msgtype: 'text' | 'markdown' | 'stream' | WeWorkNativeReplyType;
  text?: {
    content: string;
  };
  markdown?: {
